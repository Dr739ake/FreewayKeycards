...
SWEP.PrintName 				= "emptyswep1"
SWEP.Author					= "Jonas🍉"
SWEP.Purpose				= "Türen öffnen"
SWEP.Category               = "Freeway"
...
SWEP.IsBiometric = false
SWEP.OriginalOwner = nil
...
if SERVER then
	...
	hook.Add( "onDarkRPWeaponDropped", "guthscpkeycard:dropmodel", function( ply, ent, weapon )
		if weapon.Base ~= "freeway_keycard_base" then return end
...
function SWEP:run_init()
	...
	self.GuthSCPRenderer.world_model.skin = self.GuthSCPRenderer.world_model.skin or 0
	...
	self.GuthSCPRenderer.view_model.skin = self.GuthSCPRenderer.view_model.skin or 0
	...
	if self.GuthSCPRenderer.view_model.swep_ck.enabled then
		...
		--  auto-skin
		self.VElements["keycard"].skin = self.GuthSCPRenderer.view_model.skin or self.VElements["keycard"].skin or 0
	...
	if self.GuthSCPRenderer.world_model.swep_ck.enabled then
		...
		--  auto-skin
		self.WElements["keycard"].skin = self.GuthSCPRenderer.world_model.skin or self.WElements["keycard"].skin or 0
...
function SWEP:Initialize()
	self:run_init()
	if self.IsBiometric and self.OriginalOwner == nil then
